...
function _show(msg, { bg='var(--navy)', color='#fff', icon='' } = {}) {
  ...
  Object.assign(t.style, {
    ...
    padding:'11px 18px', borderRadius:'10px', background:bg, color,
    fontSize:'13px', fontWeight:'500', fontFamily:'var(--font-body)',
    boxShadow:'0 4px 20px rgba(0,0,0,0.18)',
    animation:'fadeUp .25s ease', opacity:'1', transition:'opacity .3s ease',
...
export const Toast = {
  ...
  success: (msg) => _show(msg, { bg:'var(--teal)',   icon:'✓' }),
  error:   (msg) => _show(msg, { bg:'var(--rose)',   icon:'✕' }),
  info:    (msg) => _show(msg, { bg:'var(--blue)',   icon:'ℹ' }),
  warning: (msg) => _show(msg, { bg:'var(--amber)',  icon:'⚠', color:'var(--navy-deep)' })
};